...
const getPermissions = (state) => state.auth.user.permissions;

export const selectNavigation = createSelector(
  [selectNavigationAll, ({ i18n }) => i18n.language, getPermissions],
  (navigation, language, permissions) => {
    ...
    return setTranslationValues(
      _.merge(
        [],
        filterRecursively(navigation, (item) => FuseUtils.hasPermission(item.auth, permissions))
      )
    );
  }
);
