...
import {downloadSlip, getEmployeeSalary, selectEmployeeSalary} from '../store/employeeSalarySlice';
import EmployeeSalaryListTableHead from './EmployeeSalaryListTableHead';
import FuseProgress from '../../Components/FuseProgress';
import Tooltip from "@mui/material/Tooltip";
import {Icon} from "@mui/material";

function EmployeeSalaryListTable(props) {
  const dispatch = useDispatch();
  const employees = useSelector(selectEmployeeSalary);
  const user = useSelector(({ auth }) => auth.user);
  const [selected, setSelected] = useState();
  const totalCount = useSelector(({ salaryApp }) => salaryApp.empSalary.totalCount);
  ...
  const getEmployeeData = () => {
    dispatch(getEmployeeSalary({ page, perPage: 10, employeeid: user.employee_id })).then((res) => {
      setLoading(false);
  ...
  if (data?.length === 0) {
    ...
    const DownloadPdf = (salaryId) => {
        ...
        dispatch(downloadSlip(data)).then((res) => {
            if(res){
...
export default withRouter(EmployeeSalaryListTable);
