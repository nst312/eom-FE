...
export const getEmployeeAddress = createAsyncThunk('employee/address', async (userId) => {
  ...
  async ({ id, userId, value }, { dispatch }) => {
    try {
      const response = await axios.put(
        `/api/user-address/${userId}/${id}`,
        value
      );
      const newdata = await response.data;
      dispatch(
        showMessage({
          message: 'Employee address successfully updated.',
...
export const addEmployeeAddress = createAsyncThunk(
  ...
  async ({ data, userId }, { dispatch }) => {
    try {
      const response = await axios.post(`/api/user-address/${userId}  `, data);
      const newData = await response.data;
      dispatch(
        showMessage({
